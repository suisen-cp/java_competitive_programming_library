...
public class SegmentTree<T> {
     ...
     * @param f 二項演算
     */
    public SegmentTree(int n, T e, BinaryOperator<T> f) {
        this.L = n;
        int m = 1;
        while (m < n) m <<= 1;
        this.N = m;
     ...
     * @param f 二項演算
     */
    public SegmentTree(T[] src, T e, BinaryOperator<T> f) {
        this(src.length, e, f);
     ...
     * @param len 配列のサイズ
     */
    private void build(T[] src, int len) {
    ...
    public T fold(int l, int r) {
        ...
        T resL = E, resR = E;
        while (l < r) {
            if ((l & 1) == 1) resL = F.apply(resL, Dat[l++]);
            if ((r & 1) == 1) resR = F.apply(Dat[--r], resR);
            l >>= 1; r >>= 1;
        }
